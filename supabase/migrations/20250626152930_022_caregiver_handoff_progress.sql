...
drop function if exists public.get_active_incident_for_cat(uuid);

create function public.get_active_incident_for_cat(target_cat_id uuid)
returns table (
  incident_id uuid,
  cat_id uuid,
  cat_name text,
...
begin
  ...
  order by
    case
      when a.cat_reached_at is not null then 0
      when a.state = 'accepted' then 1
      when a.state = 'notified' then 2
      when a.state = 'pending' then 3
      else 4
    end,
    a.step_number,
    i.activated_at desc
